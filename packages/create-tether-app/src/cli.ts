...
export function createCli(): Command {
  ...
  program
    ...
    .option('--llm <provider>', 'LLM backend: ollama (default), local-llm, openai, custom')
...
`
    ...
    .action(async (projectName: string | undefined, options: CliOptions) => {
      ...
      try {
        // Handle --template as alias for --llm
        ...
        if (!name) {
          if (options.yes || options.dryRun) {
            ...
            console.log(chalk.dim('  Usage: npx create-tether-app my-app [options]'));
            process.exit(1);
          }
        ...
        if (!validation.valid) {
          ...
          console.log(chalk.dim('  Project names must be lowercase and can contain letters, numbers, and hyphens.'));
          process.exit(1);
        }
        ...
        // For dry-run and --yes, skip prompts and use defaults
        if (!options.yes && !options.dryRun && !template) {
        ...
        if (options.dryRun) {
          ...
          console.log(chalk.cyan(`  Include Example: ${includeExample ? 'yes' : 'no'}`));
          console.log(chalk.cyan(`  Package Manager: ${options.useNpm ? 'npm' : options.useYarn ? 'yarn' : 'pnpm'}`));
          console.log();
          console.log('Would create directory structure:');
          console.log(chalk.dim(`  ${name}/`));
          console.log(chalk.dim(`  ├── frontend/          # React + TypeScript + Vite`));
        ...
        if (error instanceof Error) {
          ...
          if (error.message.includes('already exists')) {
            console.log(chalk.dim('  Try a different project name or delete the existing directory.'));
          }
        }
        process.exit(1);
      }
